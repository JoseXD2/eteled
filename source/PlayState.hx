...
class PlayState extends MusicBeatState
{
	...
	var useBF2:Bool = false;
	public var glitched:Bool = false;
	...
	var queuedDialoguePath:String;

	private var vocals:FlxSound;
	var useDad2:Bool = false;
	...
	public var glitchedNotes:FlxTypedGroup<Note>;
	var usingGlitchedNotes:Bool = false;
	public static var glitchedStrumLineNotes:FlxTypedGroup<FlxSprite> = null;
	public static var glitchedPlayerStrums:FlxTypedGroup<FlxSprite> = null;
	public static var glitchedCpuStrums:FlxTypedGroup<FlxSprite> = null;
	...
	var screamMode:Bool = false;
	var inGlitchedHallway:Bool = false;
	...
	var everybodyShouldDance:Bool = true;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'dream-of-peace':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/dream-of-peace/dialogue'));
			case 'diagraphephobia':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/diagraphephobia/dialogue'));	
			case 'post-mortal':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/post-mortal/dialoguepart2'));	
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'mii-channel':
					{
						...
						useSetCamera = true;
						var bg:FlxSprite = new FlxSprite(-550, -230).loadGraphic(Paths.image('stageback', 'eteled'));
						if(FlxG.save.data.antialiasing)
							{
								bg.antialiasing = true;
							}
				...
				case 'mii-channel-dark':
					{
						defaultCamZoom = 0.68;
						curStage = 'mii-channel-dark';
						useSetCamera = true;
						var bg:FlxSprite = new FlxSprite(-550, -230).loadGraphic(Paths.image('blackback', 'eteled'));
						if(FlxG.save.data.antialiasing)
							{
								bg.antialiasing = true;
							}
				...
				case 'hallway':
					{
						...
						curStage = 'hallway';
						useSetCamera = true;
						var bg:FlxSprite = new FlxSprite(-360, -210).loadGraphic(Paths.image('glitchhallway', 'eteled'));
						if(FlxG.save.data.antialiasing)
							{
								bg.antialiasing = true;
							}
		...
		if (SONG.song.toLowerCase() == 'plaything')
			useBF2 = true;

		dad = new Character(100, 100, SONG.player2);
		if (useDad2) {
			dad2 = new Character(100, 100, SONG.player2, false, true);
			dad2.visible = false;
		}
		...
		if (useBF2) {
			boyfriend2 = new Boyfriend(770, 450, SONG.player1, true);
		...
		glitchedStrumLineNotes = new FlxTypedGroup<FlxSprite>();
		add(glitchedStrumLineNotes);

		playerStrums = new FlxTypedGroup<FlxSprite>();
		cpuStrums = new FlxTypedGroup<FlxSprite>();

		glitchedPlayerStrums = new FlxTypedGroup<FlxSprite>();
		glitchedCpuStrums = new FlxTypedGroup<FlxSprite>();

		generateStaticArrows(0);
		generateStaticArrows(1);
		generateGlitchArrows(0);
		generateGlitchArrows(1);
		// startCountdown();
		...
		glitchedStrumLineNotes.cameras = [camHUD];
		notes.cameras = [camHUD];
		glitchedNotes.cameras = [camHUD];
		...
		trace('starting');
		usingGlitchedNotes = true;
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				case 'dream-of-peace' | 'post-mortal':
					if (playCutscene) {
						camHUD.visible = false;
						camGame.visible = false;
						LoadingState.loadAndSwitchState(new VideoState2("assets/videos/Introfinaleteled.webm", function() {
	...
	{
		var black:FlxSprite = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 5, FlxG.height * 5, FlxColor.BLACK);
					...
					if (dunceNote.mustPress)
						...
						dunceNote.y = (strumLineNotes.members[Math.floor(Math.abs(dunceNote.noteData))].y
							+ 0.45 * (startTime - dunceNote.strumTime) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed,
								2)) - dunceNote.noteYOff;
					toBeRemoved.push(dunceNote);
					var glitchedNote = new Note(dunceNote.strumTime, dunceNote.noteData, dunceNote.prevNote, dunceNote.isSustainNote, false, dunceNote.noteSkin, true);
					if (usingGlitchedNotes) {
						glitchedNotes.add(glitchedNote);
						toBeRemoved.push(glitchedNote);
					}
				}
			}
			...
			if (dataNotes.length > 1) // stacked notes or really close ones
			{
				for (i in 0...dataNotes.length)
				{
					...
					if (!note.isSustainNote && (note.strumTime - coolNote.strumTime) < 2)
					{
						...
						if (usingGlitchedNotes)
							glitchedNotes.remove(note, true);
						note.destroy();
					}
				}
			}
	...
	function startSong():Void
	{
		...
		if (isStoryMode) {
			switch (SONG.song.toLowerCase()) {
				case 'dream-of-peace':
					queuedDialoguePath = Paths.txt('data/diagraphephobia/dialogue');
					FlxG.sound.music.onComplete = startEndDialogue;
				case 'diagraphephobia':
					queuedDialoguePath = Paths.txt('data/post-mortal/dialogue');
					FlxG.sound.music.onComplete = startEndDialogue;
				case 'post-mortal':
					queuedDialoguePath = Paths.txt('data/post-mortal/dialoguepart3');
					FlxG.sound.music.onComplete = startEndDialogue;
				default:
	...
	public function generateSong(dataPath:String):Void
	{
		...
		glitchedNotes = new FlxTypedGroup<Note>();
		add(glitchedNotes);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				default:
					if (player == 0)
						babyArrow.frames = Paths.getSparrowAtlas('notes/' + dad.noteSkin);
					if (player == 1)
						babyArrow.frames = Paths.getSparrowAtlas('notes/' + boyfriend.noteSkin);
					for (j in 0...4)
					{
	...
	private function generateGlitchArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			// FlxG.log.add(i);
			...
			if (player == 0)
				babyArrow.frames = Paths.getSparrowAtlas('notes/' + dad.noteSkin + 'Glitch');
			if (player == 1)
				babyArrow.frames = Paths.getSparrowAtlas('notes/' + boyfriend.noteSkin + 'Glitch');
			for (j in 0...4)
			{
				babyArrow.animation.addByPrefix(dataColor[j], 'arrow' + dataSuffix[j]);	
			}
			...
			{
				case 0:
					glitchedCpuStrums.add(babyArrow);
				case 1:
					glitchedPlayerStrums.add(babyArrow);
			}

			glitchedCpuStrums.forEach(function(spr:FlxSprite)
			{
				spr.centerOffsets(); // CPU arrows start out slightly off-center
			});
			babyArrow.visible = false;
			glitchedStrumLineNotes.add(babyArrow);
		}
	}
	...
	private function appearStaticArrows():Void
	{
		...
		glitchedStrumLineNotes.forEach(function(babyArrow:FlxSprite)
		{
			if (isStoryMode) 
				babyArrow.alpha = 1;
		});
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (usingGlitchedNotes) {
					glitchedNotes.members[notes.members.indexOf(daNote)].x = daNote.x;
					glitchedNotes.members[notes.members.indexOf(daNote)].y = daNote.y;
					//if (glitchedNotes.members[notes.members.indexOf(daNote)].y && )
				}
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (screamMode)
						...
						if (useDad2)
							dad2.playAnim('sing' + dataSuffix[singData] + altAnim, true);
					}
					...
					notes.remove(daNote, true);
					if (usingGlitchedNotes)
						glitchedNotes.remove(daNote, true);
					daNote.destroy();
				}
		...
		if (FlxG.keys.justPressed.G) {
			...
			new FlxTimer().start(FlxG.random.float(0.2, 0.5), function(tmr:FlxTimer)
			{
				glitched = false;
				/*for (i in glitchedNotes) {
					i.visible = false;
	...
	function startEndDialogue():Void
	{
		...
		for (i in strumLineNotes)
			i.visible = false;
		for (i in glitchedStrumLineNotes)
			i.visible = false;
		healthBar.visible = false;
		everybodyShouldDance = false;
		healthBarBG.visible = false;
		iconP2.visible = false;
		iconP1.visible = false;
		camZooming = false;
		...
		FlxG.sound.music.stop();
		vocals.stop();
		var dialogue = CoolUtil.coolTextFile(queuedDialoguePath);
		var doof:DialogueBox = new DialogueBox(false, dialogue, '2');
		doof.scrollFactor.set();
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				if (useBF2) {
					if (boyfriend2.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || PlayStateChangeables.botPlay))
						{
							if (boyfriend2.animation.curAnim.name.startsWith('sing') && !boyfriend2.animation.curAnim.name.endsWith('miss') && boyfriend2.animation.curAnim.curFrame >= 10)
								boyfriend2.playAnim('idle');
						}
				}
		...
		glitchedPlayerStrums.forEach(function(spr:FlxSprite)
		{
			if (keys[spr.ID] && spr.animation.curAnim.name != 'confirm')
				spr.animation.play('pressed', false);
			if (!keys[spr.ID])
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (useBF2)
				boyfriend2.playAnim('sing' + dataSuffix[direction] + 'miss', true);
	...
	override function stepHit()
	{
		...
		switch (SONG.song.toLowerCase()){
			case 'diagraphephobia':
				...
				if (inGlitchedHallway) {
					var glitchChance = FlxG.random.int(0, 15);
					trace(glitchChance);
					if (glitchChance == 7) {
						randomForegroundGlitch();
					} else 
						if (glitchChance == 8) {
						randomGlitch();
					} 
			...
			case 'post-mortal':
				var glitchChance2 = FlxG.random.int(0, 20);
				if (glitchChance2 == 7) {
					randomForegroundGlitch();
				} else 
					if (glitchChance2 == 8) {
				...
				var glitchChance = FlxG.random.int(0, 13);
				if (glitchChance == 7) {
					glitched = true;
					...
					new FlxTimer().start(FlxG.random.float(0.1, 0.5), function(tmr:FlxTimer)
					{
						dad.visible = true;
						dad2.visible = false;
						glitched = false;
	...
	function randomGlitch() {
		var deRandom = FlxG.random.int(0, 4);
		glitchLayer.clear();
		var elGlitch:FlxSprite = new FlxSprite(-FlxG.width, -FlxG.height);
		switch (deRandom) {
			case 0:
				elGlitch.frames = Paths.getSparrowAtlas('glitch effects/glitchAnim', 'eteled');
				elGlitch.animation.addByPrefix('idle', 'g', 24, true);
				elGlitch.scale.set(8, 8);
			case 1:
				elGlitch.frames = Paths.getSparrowAtlas('glitch effects/noise2', 'eteled');
				elGlitch.animation.addByPrefix('idle', 'f', 24, true);
				elGlitch.scale.set(8, 8);
			case 2:
				elGlitch.frames = Paths.getSparrowAtlas('glitch effects/noise2R', 'eteled');
				elGlitch.animation.addByPrefix('idle', 'f', 24, true);
				elGlitch.scale.set(8, 8);
			case 3:
				elGlitch.frames = Paths.getSparrowAtlas('glitch effects/sheet', 'eteled');
				elGlitch.animation.addByPrefix('idle', 'Idle', 24, true);
				elGlitch.scale.set(8, 8);
			/*case 4:
				foregroundGlitch = new FlxSprite(0, 0);
				foregroundGlitch.frames = Paths.getSparrowAtlas('glitch effects/sheeto2', 'eteled');
				foregroundGlitch.animation.addByPrefix('idle', 'n', 24, true);
				foregroundGlitch.scale.set(8, 8);*/
		}
		
		elGlitch.alpha = FlxG.random.float(0.1, 0.2);
		//elGlitch.cameras = [camHUD];
		elGlitch.animation.play('idle');
		glitchLayer.add(elGlitch);
		new FlxTimer().start(FlxG.random.float(0.2, 0.5), function(tmr:FlxTimer)
		{
			if (glitchLayer.members.contains(elGlitch))
				glitchLayer.remove(elGlitch);
		});
	}
	function randomForegroundGlitch() {
		var deRandom = FlxG.random.int(0, 4);
		foregroundGlitchLayer.clear();
		var FG = new FlxSprite(0, 0);
		switch (deRandom) {
			case 0:
				
				FG.frames = Paths.getSparrowAtlas('glitch effects/glitchAnim', 'eteled');
				FG.animation.addByPrefix('idle', 'g', 24, true);
				FG.scale.set(4, 4);
			case 1:
				FG.frames = Paths.getSparrowAtlas('glitch effects/noise2', 'eteled');
				FG.animation.addByPrefix('idle', 'f', 24, true);
				FG.scale.set(4, 4);
			case 2:
				FG.frames = Paths.getSparrowAtlas('glitch effects/noise2R', 'eteled');
				FG.animation.addByPrefix('idle', 'f', 24, true);
				FG.scale.set(4, 4);
			case 3:
				FG.frames = Paths.getSparrowAtlas('glitch effects/sheet', 'eteled');
				FG.animation.addByPrefix('idle', 'Idle', 24, true);
				FG.scale.set(8, 8);
			/*case 4:
				foregroundGlitch = new FlxSprite(0, 0);
				foregroundGlitch.frames = Paths.getSparrowAtlas('glitch effects/sheeto2', 'eteled');
		...
		FG.alpha = FlxG.random.float(0.1, 0.2);
		FG.cameras = [camHUD];
		FG.animation.play('idle');
		foregroundGlitchLayer.add(FG);
		new FlxTimer().start(FlxG.random.float(0.2, 0.5), function(tmr:FlxTimer)
		{
			if (foregroundGlitchLayer.members.contains(FG))
				foregroundGlitchLayer.remove(FG);
		});
	}

	function randomForegroundTransGlitch() {
		var deSound = FlxG.sound.play(Paths.sound('Glitch-Transition-Sound-Effect', 'eteled'), 0.6);
		var deRandom = FlxG.random.int(0, 4);
		foregroundGlitchLayer.clear();
		foregroundGlitch = new FlxSprite(0, 0);
		foregroundGlitch.frames = Paths.getSparrowAtlas('glitch effects/glitchAnim', 'eteled');
		foregroundGlitch.animation.addByPrefix('idle', 'g', 24, true);
		foregroundGlitch.scale.set(2, 2);
			/*case 4:
				foregroundGlitch = new FlxSprite(0, 0);
				foregroundGlitch.frames = Paths.getSparrowAtlas('glitch effects/sheeto2', 'eteled');
				foregroundGlitch.animation.addByPrefix('idle', 'n', 24, true);
				foregroundGlitch.scale.set(8, 8);*/
		foregroundGlitch.cameras = [camHUD];
		foregroundGlitch.animation.play('idle');
		foregroundGlitchLayer.add(foregroundGlitch);
	}
	...
	function switchToHallway() {
		...
		curStage = 'hallway';
		boyfriend.setPosition(1355, 515);
		gf.setPosition(580, 110);
		remove(dad);
		dad = new Character(97, 323, 'eteled3');
		add(dad);
		miiButtons.visible = false;
		channelOverlay.visible = false;
		//dad.setPosition(97 , 323);
		inGlitchedHallway = true;
	}

	function switchOutHallway() {
		...
		curStage = 'mii-channel-glitch';
		boyfriend.setPosition(1365, 505);
		gf.setPosition(580, 130);
		remove(dad);
		dad = new Character(127, 303, SONG.player2);
		add(dad);
	...
	override function beatHit()
	{
		...
		switch (SONG.song.toLowerCase()) {
			case 'diagraphephobia':
				switch (curBeat) {
					case 111:
						randomForegroundTransGlitch();
						switchToHallway();
					case 112:
						foregroundGlitchLayer.clear();
						//stop gltich be in hallway
					case 152:
						randomForegroundTransGlitch();
						switchOutHallway();
					case 153:
		...
		if (generatedMusic)
		{
			...
			if (usingGlitchedNotes)
				glitchedNotes.sort(FlxSort.byY, (PlayStateChangeables.useDownscroll ? FlxSort.ASCENDING : FlxSort.DESCENDING));
		}
		...
		if (useBF2) {
			if (!boyfriend2.animation.curAnim.name.startsWith("sing") && (curBeat % idleBeat == 0 || !idleToBeat))
			{
				if (everybodyShouldDance)
					boyfriend2.playAnim('idle', idleToBeat);
			}
		}
